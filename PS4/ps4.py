...
def generate_models(x, y, degs):
    ...
    numarrays = []
    for degree in degs:
        numarrays.append(np.polyfit(x, y, degree))
    return numarrays
...
def r_squared(y, estimated):
    ...
    y, estimated = np.array(y),np.array(estimated) # array of the 2 lists
    error = ((estimated - y)**2).sum()
    meansError = error/len(y)
    return 1 - (meansError/np.var(y))
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in range(len(models)):
        ...
        pylab.plot(x, estYVals,color='r',label = 'Model = ' + str(np.round(models[i])) +
